...
export default function FeaturedCollection({ products }) {
  ...
  return (
    <section className='mt-5 md:relative'>
      ...
      <div className='bg-primary-5 text-[white] grid grid-cols-autofill-sm max-w-6xl md:my-12 my-6 mx-auto p-2'>
...
const SingleProduct = ({
  ...
  return (
    <article className='font-medium relative text-xs md:text-sm text-primary-8 bg-[white] m-2 rounded-md  p-4'>
      <Link href={`/${productId}`}>
        <a className='flex flex-col items-center'>
          <img src={imageUrl} alt={name} className='whitespace-nowrap' />
          <h3 className='p-3 text-xs md:text-sm'>{name}</h3>
